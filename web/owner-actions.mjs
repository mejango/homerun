...
function stickyPolicy(projection) {
  return {
    ...
    rewardRoute: {
      ...
      beneficiaryAddress: null,
      notBeneficiary: ['raw FH-FUND token', 'StickyHook'],
    },
  ...
  if (action === 'complete_purchase') {
    ...
    draft.steps.push(
      ...
      ...(holderRewards ? [
        ...
        step('configure_sticky_reward_route', 'Route the ongoing FUND-staker allocation', 'Configure the reviewed total reserved issuance and split it between operations and FUND stakers. At 75% operations plus 15% for stakers, reserve 90%; split that reserve 75/90 and 15/90 with reviewed integer rounding. The remaining 10% of total issuance goes to customers. Leave every split unlocked so the Owner can change recipients and allocations. The reward split hook is JBTokenDistributor and its beneficiary is the FUND Sticky SHARE ERC20, never raw FUND or StickyHook.', { rewardRoute: draft.changes.fundSticky.rewardRoute, revenuePolicy: draft.changes.revenuePolicy }),
      ]),
