...
| [src/HomerunDeployer.sol](src/HomerunDeployer.sol), [src/HomerunAllowlistHook.sol](src/HomerunAllowlistHook.sol), [src/interfaces](src/interfaces), [src/structs](src/structs) | Fixed-rule FUND launches, the owner-managed payment allowlist (delegable to operators through `JBPermissions`), and the initial INCOME allocation recorded for the FUND owner |
