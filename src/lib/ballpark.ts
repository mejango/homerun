export type BallparkMotion = 'running' | 'paused' | 'reduced';
export type BallparkOptions = {
  mode: 'acid' | 'shapes';
  intensity: number;
  pace: number;
  grouping: number;
};
export type BallparkController = {
  setOptions: (options: BallparkOptions) => void;
  toggleMotion: () => void;
  destroy: () => void;
};
type Scene = {
  ...
  compact: boolean;
  skyPadding: number;
  textRects: Array<{ left: number; right: number; top: number; bottom: number }>;
  saucer: { x: number; y: number; scale: number };
};
function context2d(canvas: HTMLCanvasElement): CanvasRenderingContext2D {
  const context = canvas.getContext('2d');
  if (!context) throw new Error('The ballpark needs a 2D canvas context.');
  return context;
}
...
export function mountBallpark(
  canvas: HTMLCanvasElement,
  copy: HTMLElement,
  initialOptions: BallparkOptions,
  onMotionChange: (state: BallparkMotion) => void,
): BallparkController {
  ...
  const path = (points: number[][]) => { const p = new Path2D(); points.forEach(([x, y], i) => i ? p.lineTo(x, y) : p.moveTo(x, y)); p.closePath(); return p; };
  const oval = (x: number, y: number, rx: number, ry: number) => { const p = new Path2D(); p.ellipse(x, y, rx, ry, 0, 0, Math.PI * 2); return p; };
  let grain: CanvasPattern | string = C.paper;
  let players: Array<{ sprite: HTMLCanvasElement; x: number; y: number; phase: number }> = [];
  const still = document.createElement('canvas');
  const stillContext = context2d(still);
  const shapeSurface = document.createElement('canvas');
  const shapeContext = context2d(shapeSurface);
  let shapeIndex = 0;
  let shapeGroup: number | null = null;
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  const FRAME_INTERVAL = 1000 / 24;
  let { mode: colorMode, intensity: colorIntensity, pace: colorPace, grouping: colorGrouping } = initialOptions;
  let motionPaused = false, motionRequested = false;
  let scene: Scene | undefined;
  let animationFrame = 0, resizeFrame = 0, lastTick = 0, lastPaint = 0, motionTime = 0;
  let inView = true, pageActive = true;
  let previousMotionState: BallparkMotion | undefined;
  ...
  function motionAllowed() {
    return !disposed && scene && !motionPaused && (!reducedMotion.matches || motionRequested) && !document.hidden && inView && pageActive;
  }

  function syncMotion() {
    if (disposed) return;
    const state: BallparkMotion = reducedMotion.matches && !motionRequested ? 'reduced' : motionAllowed() ? 'running' : 'paused';
    canvas.dataset.motion = state;
    if (state !== previousMotionState) {
      previousMotionState = state;
  ...
  function tickMotion(now: number) {
    animationFrame = 0;
  ...
  function flutteringFlag(x: number, y: number, w: number, h: number, color: string, time: number, phase: number) {
  ...
  function flyingBird(x: number, y: number, size: number, time: number, phase: number) {
  ...
  function coffeeSteam(time: number) {
    if (!scene) return;
    const { saucer } = scene;
  ...
  function livingField(time: number) {
  ...
  function paintScene(time: number) {
    if (!scene) return;
  ...
  let colorTexture: WebGLTexture | null = null, shapeTexture: WebGLTexture | null = null;
  let colorTime: WebGLUniformLocation | null = null, colorAmplitude: WebGLUniformLocation | null = null;
  let colorGroupingUniform: WebGLUniformLocation | null = null, colorModeUniform: WebGLUniformLocation | null = null;
  ...
  function prepareColorDrift() {
    ...
    if (!colorProgram) {
      ...
      if (!colorProgram) { gl.deleteShader(vertex); gl.deleteShader(fragment); return; }
      gl.attachShader(colorProgram, vertex); gl.attachShader(colorProgram, fragment); gl.linkProgram(colorProgram);
      gl.deleteShader(vertex); gl.deleteShader(fragment);
      if (!gl.getProgramParameter(colorProgram, gl.LINK_STATUS)) { gl.deleteProgram(colorProgram); colorProgram = null; return; }
  ...
  function paintMotion(time: number) {
    if (!scene) return;
    ctx.setTransform(1,0,0,1,0,0);
    if (reducedMotion.matches && !motionRequested) { ctx.drawImage(still,0,0); return; }
    if (colorProgram && colorGL && !colorGL.isContextLost()) {
  ...
  function protectedTextRects(bounds: DOMRect, scale: number, offset: number) {
  ...
  function pencilLine(points: number[][], color = C.ink, width = 1, passes = 2) {
  ...
  function shade(p: Path2D, color: string, outline = true) {
  ...
  function hatching(p: Path2D, bounds: number[], color: string, count = 70, direction = 1) {
  ...
  function shrub(x: number, y: number, w: number, h: number, color = C.green) {
  ...
  function tree(...args: Parameters<typeof drawTree>) {
  ...
  function drawTree(x: number, ground: number, size: number, color = C.green) {
  ...
  function windowAt(x: number, y: number, w: number, h: number, shutters = false) {
  ...
  function archedWindowAt(x: number, y: number, w: number, h: number, shutters = false) {
    const p = new Path2D();
  ...
  function balcony(x: number, y: number, w: number, color = C.cream) {
  ...
  function house(...args: Parameters<typeof drawHouse>) {
    const previous = shapeGroup;
    shapeGroup = Math.abs(Math.round(args[0] * 17 + args[1] * 31)) % 256;
    drawHouse(...args);
    shapeGroup = previous;
  ...
  function drawHouse(x: number, base: number, w: number, h: number, color: string, roofColor = C.red, style = 'cottage') {
    const y = base - h, side = w * .18, roof = h * .32;
    const poly = (points: number[][], fill: string) => shade(path(points), fill);
    const line = (points: number[][], fill = C.ink, weight = 1) => pencilLine(points, fill, weight);
    const door = (dx: number, dy: number, dw: number, dh: number, fill = C.dark) => {
      poly([[dx, dy], [dx + dw, dy], [dx + dw, dy + dh], [dx, dy + dh]], fill);
      line([[dx + dw * .78, dy + dh * .56], [dx + dw * .78, dy + dh * .61]], C.yellow, 1.8);
    };
    const chimney = (cx: number, cy: number, ch = 20) => {
      poly([[cx, cy], [cx, cy - ch], [cx + 8, cy - ch], [cx + 8, cy + 4]], C.red);
      line([[cx - 1, cy - ch], [cx + 9, cy - ch]], C.cream, 1.6);
    };
    const porch = (px: number, py: number, pw: number, posts = 3, fill = C.cream) => {
    ...
    const gable = (gx: number, gy: number, gw: number, gh: number, fill: string) => {
  ...
  function storefront(x: number, base: number, w: number) {
  ...
  function tractor(x: number, y: number, scale = 1) {
  ...
  function coffeeSaucer(x: number, y: number, scale = 1) {
  ...
  function bench(x: number, y: number, scale = 1, teamColor = C.cream, facing = 1) {
    ctx.save(); ctx.translate(x, y); ctx.scale(scale, scale);
  ...
  function seatedPlayer(x: number, y: number, color: string, scale: number, facing: number, pose: number) {
  ...
  function player(x: number, y: number, color = C.red, s = 1, pose = 0) {
  ...
  function coastline(visibleWidth: number) {
    ...
    const lx = (f: number) => cliffLeft + cliffWidth * f;
    ...
    const ex = (f: number) => eastX + eastWidth * f;
  ...
  function draw() {
    ...
    grain = ctx.createPattern(texture, 'repeat') ?? C.paper;
    ...
    players = ([[800,687,C.red,.85,0], [770,841,C.cream,1.02,1],
      ...
      [523,554,C.red,.64,0], [1120,578,C.red,.64,0]] satisfies Array<[number, number, string, number, number]>).map(([x,y,color,size,pose], index) => {
  ...
  const pageHide = () => { pageActive = false; syncMotion(); };
  const pageShow = () => { pageActive = true; syncMotion(); };
  ...
  const contextLost = (event: Event) => { event.preventDefault(); colorProgram = null; };
  ...
  colorSurface.addEventListener('webglcontextlost', contextLost);
  colorSurface.addEventListener('webglcontextrestored', contextRestored);
  watchResolution();
  draw();
  ...
  return {
    setOptions(options) {
      ...
      colorIntensity = options.intensity;
      colorPace = options.pace;
      colorGrouping = options.grouping;
      paintMotion(motionTime);
    },
    toggleMotion() {
      if (disposed) return;
    ...
    destroy() {
      ...
      resolution?.removeEventListener('change', resolutionChanged);
      colorSurface.removeEventListener('webglcontextlost', contextLost);
      colorSurface.removeEventListener('webglcontextrestored', contextRestored);
      if (colorGL) {
        colorGL.deleteTexture(colorTexture);
        colorGL.deleteTexture(shapeTexture);
        colorGL.deleteBuffer(vertexBuffer);
        colorGL.deleteProgram(colorProgram);
        colorGL.getExtension('WEBGL_lose_context')?.loseContext();
      }
      still.width = 0;
      still.height = 0;
      shapeSurface.width = 0;
      shapeSurface.height = 0;
      colorSurface.width = 0;
      colorSurface.height = 0;
      players = [];
      scene = undefined;
