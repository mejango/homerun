...
type Listable = { id: string; name: string; icon?: string | undefined; type?: string | undefined };
...
export function offerableWallets<T extends Listable>(connectors: readonly T[]): T[] {
  ...
  const usable = hasDiscoveredInjected
    ? external.filter((connector) => connector.id !== 'injected')
    : external;
