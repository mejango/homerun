...
import { bytesToHex, decodeEventLog, decodeFunctionData, encodeFunctionData, formatUnits, isAddressEqual, type Hex, type PublicClient } from 'viem'
...
import { buildFundGlobalManifest, canonicalSnapshotJson, fundGlobalManifestHash, globalIncomeSnapshotParameters, parseFundGlobalManifest, serializeFundGlobalManifest, verifyFundGlobalManifestHistory, type FundGlobalManifest } from '@/lib/fund-global-manifest'
...
function GlobalIncomeLaunch({ state, client, name = 'Homerun INCOME', launchUnavailable = false, plannedAllocation, embedExistingProject = true }: IncomeLaunchProps) {
  ...
  return <section className="mt-7 rounded-md border border-[#c4cdbb] bg-[#eef1e7] p-5 sm:p-7">
    ...
    <p className="mb-4 text-sm">Each chain records its share as an auto-issuance that anyone can mint to the FUND owner, who settles it to snapshot holders per the published allocation. New INCOME issues at 10 per dollar, cutting 2% every quarter; cash outs are taxed 10%. The owner holds the reserved share until it is split further.</p>
...
function IncomeChainLaunch({ local, draft, manifest, clients, rootState, onVerified, launchUnavailable, rootIncomeId, embedExistingProject }: { local: IncomeGlobalChainDraft; draft: IncomeGlobalLaunchDraft; manifest: FundGlobalManifest; clients: ReadonlyMap<number, PublicClient>; rootState: FundProjectState; onVerified: (chainId: number, hash: string) => void; launchUnavailable: boolean; rootIncomeId: bigint | null; embedExistingProject: boolean }) {
  ...
  return <IncomeChainActions local={local} draft={draft} manifest={manifest} clients={clients} client={client} rootState={rootState} onVerified={onVerified} launchUnavailable={launchUnavailable} rootIncomeId={rootIncomeId} embedExistingProject={embedExistingProject} />
}

function IncomeChainActions({ local, draft, manifest, clients, client, rootState, onVerified, launchUnavailable, rootIncomeId, embedExistingProject }: { local: IncomeGlobalChainDraft; draft: IncomeGlobalLaunchDraft; manifest: FundGlobalManifest; clients: ReadonlyMap<number, PublicClient>; client: PublicClient; rootState: FundProjectState; onVerified: (chainId: number, hash: string) => void; launchUnavailable: boolean; rootIncomeId: bigint | null; embedExistingProject: boolean }) {
  ...
  const confirm = useCallback(async (record: IncomeLaunchPending, hash: Hex) => {
    ...
    if (status === 'confirmed') {
      ...
      if (allocation.blockNumber < receipt.blockNumber) throw new Error('The recorded initial allocation could not be verified yet.')
      assertInitialIncomeAllocation(allocation, BigInt(local.initialIncomeAmount))
  ...
  }, [cache, chainId, client, draft, key, local.execution, local.fundProjectId, local.initialIncomeAmount, manifest, onVerified, projectId])
  useEffect(() => {
  ...
  return <section className="rounded border border-[#c4cdbb] p-4 sm:p-5" aria-label={`${displayChainName(chainId)} INCOME launch`}>
    ...
    {verifiedId && !(rootState.chainId === chainId && rootIncomeId === verifiedId) && (embedExistingProject ? <IncomeProject chainId={chainId} projectId={verifiedId} fundProjectId={projectId} /> : <a className="inline-block underline" href={`/income/${chainId}/${verifiedId}`}>Open INCOME on {displayChainName(chainId)} →</a>)}
  </section>
}
