...
export function FundingProgress({ raised, goal, historical = false, compact = false }: { raised: number; goal: number; historical?: boolean; compact?: boolean }) {
  ...
  return <figure className={`${styles.chart}${compact ? ` ${styles.compact}` : ''}`} aria-label={historical ? 'Fundraising result' : 'Fundraising progress'}>
    <div
      className={styles.track}
      role={goal > 0 ? 'progressbar' : 'img'}
      aria-label={goal > 0 ? compact ? 'Project funding meter' : historical ? 'Funds raised against the goal' : 'Funding goal progress' : valueText}
    ...
    {!compact && <figcaption className={styles.legend}>
