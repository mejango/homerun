...
import {
  demoShopPrice, MAX_DEMO_SHOP_MEDIA_BYTES, newDemoShopItem, validateDemoShopItem,
  type DemoShopCurrency, type DemoShopItem, type DemoShopSplit,
} from '@/lib/demo-shop'
import './demo-project-shop.css'

export type ShopItemEditorState = {
  mode: 'add' | 'edit'
  items: DemoShopItem[]
  /** Inherited from the shop; a new shop uses its launch currency. */
  currency: DemoShopCurrency
}
type ShopItemEditorVariant = 'demo' | 'live'

/** Shared draft/review UI. The caller handles local saving or live publication after review. */
export function ShopItemEditor({ initial, categories, maximumItems, variant = 'demo', priceDecimals, onClose, onSave }: {
  initial: ShopItemEditorState; categories: string[]; maximumItems: number; variant?: ShopItemEditorVariant; priceDecimals?: number; onClose: () => void
  ...
  const errors = items.map(item => ({ ...(attempted ? validateDemoShopItem(item, currency, priceDecimals) : {}), ...(mediaErrors[item.id] ? { media: mediaErrors[item.id] } : {}) }))
  useEffect(() => {
    if (review) editorRef.current?.querySelector<HTMLElement>('.ds-item-review')?.focus({ preventScroll: true })
  }, [review])
  ...
  async function upload(itemId: string, file: File | undefined) {
    ...
    if (file.size > MAX_DEMO_SHOP_MEDIA_BYTES) {
      setMediaErrors(current => ({ ...current, [itemId]: 'Use a file up to 2 MB, or add a media URL.' }))
      return
    }
    setUploading(itemId)
  ...
  function submit(event: FormEvent) {
    ...
    if (items.length > maximumItems) { setMessage(`Review up to ${maximumItems} ${maximumItems === 1 ? 'item' : 'items'} at a time. Remove a draft to continue.`); return }
    const invalid = items.findIndex(item => Object.keys(validateDemoShopItem(item, currency, priceDecimals)).length > 0 || mediaErrors[item.id])
    if (invalid >= 0) {
    ...
    setMessage('')
    setReview(true)
    editorRef.current?.closest('dialog')?.querySelector<HTMLElement>('[data-modal-body]')?.scrollTo({ top: 0 })
  }
  ...
  return <ModalShell
    title={discard ? 'Discard your changes?' : review ? initial.mode === 'edit' ? 'Review item changes' : 'Review shop items' : initial.mode === 'edit' ? demo ? 'Edit demo item' : 'Edit item' : 'Add items for sale'}
    subtitle={discard ? demo ? 'Your existing demo inventory will stay as it is.' : 'Your existing inventory will stay as it is.' : review ? demo ? 'Check each item before saving it to your demo inventory.' : 'Check each item before continuing to the transaction.' : 'Stage your items, then review how they will appear in the shop.'}
    ...
    footer={<div className="ds-footer">
      {discard ? <button type="button" className="ds-button ds-button-secondary" onClick={() => setDiscard(false)}>Keep editing</button>
        : review ? <><button type="button" className="ds-button ds-button-secondary" onClick={() => setReview(false)}>Back to editor</button><button type="button" className="ds-button ds-button-primary" onClick={() => onSave(items.map(item => ({ ...item, name: item.name.trim(), description: item.description.trim(), category: item.category.trim(), splits: item.splits.map(split => ({ ...split })) })), currency)}>{!demo ? 'Continue to transaction' : initial.mode === 'edit' ? 'Save demo changes' : items.length === 1 ? 'Add item to demo shop' : `Add ${items.length} items to demo shop`}</button></>
  ...
  >
    <div ref={editorRef} className="demo-shop-editor">
      {discard ? <p>Your unsaved item drafts will be discarded. Nothing has been published or sent onchain.</p> : <>
        {demo && <p className="ds-editor-notice"><strong>Local demo</strong> {review ? 'Saving adds previews on this device. There is no upload, wallet signature, or onchain transaction.' : 'Try the same item settings used by Juicebox shops. Nothing is uploaded or published.'}</p>}
        {review ? <div className="ds-review-list">{items.map((item, index) => <section key={item.id} aria-label={`Review item ${index + 1}`} className="ds-review-item"><p className="ds-eyebrow">Item {index + 1}</p><ShopItemReview item={item} currency={currency} priceDecimals={priceDecimals} /></section>)}</div>
          : <form id={formId} onSubmit={submit} noValidate>
            {items.map((item, index) => <DraftEditor key={item.id} variant={variant} item={item} index={index} currency={currency} categories={[...new Set([...categories, ...items.map(entry => entry.category.trim()).filter(Boolean)])]} errors={errors[index]} validationAttempt={attempted} uploading={uploading === item.id} disabled={!!uploading} onUpdate={patch => update(item.id, patch)} onUpload={file => void upload(item.id, file)} onRemove={initial.mode === 'add' ? () => { setItems(current => current.filter(entry => entry.id !== item.id)); setMediaErrors(current => ({ ...current, [item.id]: '' })); setMessage('') } : undefined} onClearMediaError={() => setMediaErrors(current => ({ ...current, [item.id]: '' }))} />)}
            ...
            {initial.mode === 'add' && items.length >= maximumItems && <p className="ds-help">{demo ? maximumItems === 20 ? 'Review up to 20 items at a time in this demo.' : `This demo has room for ${maximumItems} more ${maximumItems === 1 ? 'item' : 'items'}.` : `Review up to ${maximumItems} ${maximumItems === 1 ? 'item' : 'items'} at a time.`}</p>}
...
function DraftEditor({ variant, item, index, currency, categories, errors, validationAttempt, uploading, disabled, onUpdate, onUpload, onRemove, onClearMediaError }: {
  variant: ShopItemEditorVariant; item: DemoShopItem; index: number; currency: DemoShopCurrency; categories: string[]; errors: Record<string, string>; validationAttempt: number; uploading: boolean; disabled: boolean
  ...
  return <fieldset className="ds-draft-item" disabled={disabled}>
    ...
    <div className="ds-item-basics">
      <div className="ds-media-field">
        <ShopMediaPreview media={item.media} />
      ...
      <div className="ds-basics-fields"><div className="ds-field"><label htmlFor={`${id}-name`}>Item name</label><input {...field('name')} type="text" autoComplete="off" placeholder={variant === 'demo' ? 'e.g. A night at Founder Haus' : 'e.g. A weekend stay'} value={item.name} maxLength={100} onChange={event => onUpdate({ name: event.target.value })} />{error('name')}</div><div className="ds-field-pair">{numberField('price', `Price (${currency})`, currency === 'USD' ? '25' : '0.01')}{numberField('supply', 'Quantity', 'Unlimited', true)}</div><p className="ds-help">Leave quantity blank for unlimited inventory.</p></div>
    </div>
    <div className="ds-media-options"><button type="button" className="ds-text-button" aria-expanded={mediaMode === 'url'} aria-controls={`${id}-media-url`} onClick={() => setMediaMode(mediaMode === 'url' ? 'upload' : 'url')}>{mediaMode === 'url' ? 'Hide media URL' : 'Use a media URL'}</button><span>Images, video, audio, PDF, or text | Up to 2 MB</span></div>
...
export function ShopItemReview({ item, currency, priceDecimals }: { item: DemoShopItem; currency: DemoShopCurrency; priceDecimals?: number }) {
  return <div className="ds-item-review" tabIndex={0} role="region" aria-label={`${item.name} details`}>
    <div className="ds-review-top"><ShopMediaPreview media={item.media} large /><div><h3>{item.name}</h3>{item.description && <p className="ds-review-description">{item.description}</p>}<p className="ds-review-price">{demoShopPrice(item, currency, priceDecimals)}</p>{Number(item.discountPct) > 0 && <p className="ds-help">Listed at {item.price} {currency} | {item.discountPct}% off</p>}</div></div>
...
export function ShopMediaPreview({ media, large = false }: { media: DemoShopItem['media']; large?: boolean }) {
