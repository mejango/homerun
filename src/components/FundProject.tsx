...
function ProjectActions({ chainId, projectId, state, client, details, notice, income, refreshing, writesUnavailable, refresh }: {
  ...
  if (!state || !client) {
    ...
    return <HomerunProjectLayout title={details?.name ?? 'FUND project'}
      ...
      overview={<div className="grid gap-7"><ActionSection title="About"><p>{details?.description ?? 'Fund the asset, manage its treasury, and use your FUND tokens.'}</p>{pending}</ActionSection>{details?.operator && <OperatorProfile {...details.operator} />}</div>}
      stages={pending}
