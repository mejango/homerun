...
All stages retain global cash-out accounting and the normal Sucker deployment/retry bit. Issuance begins at 10 INCOME/USD, falls 5% every 7,884,000 seconds for eight cuts, then stays fixed. The default reserved 80% splits 70/80 to operations and 10/80 to the local Sticky SHARE rewards. Every split row has `lockedUntil = 0`. The Owner can change all split recipients and allocations. Stock Revnet identity excludes individual split weights and routing; authorized changes on already-launched peers do not block completion of the initial linked launch plan.
