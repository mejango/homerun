...
The FUND Owner launches INCOME and becomes its stock Revnet control wallet (`REVConfig.operator`), while canonical `REVOwner` holds the INCOME NFT. The Owner supplies the INCOME name and ticker and the reserved percent of new issuance (0–100%). The deployer routes the entire reserved share to one unlocked split (`lockedUntil = 0`) held by the Owner, who can replace recipients or change allocations later using the stock controller's `setSplitGroupsOf`. `deployIncome` takes `(fundProjectId, snapshot, description, reservedBps, startsAtOrAfter, suckerConfiguration)`; INCOME only attaches to a FUND the same deployer launched.
