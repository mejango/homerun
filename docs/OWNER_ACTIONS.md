...
This document describes the demo's downloadable, non-executable drafts. The native live FUND operator controls and transaction verification are documented in [TRANSACTIONS.md](TRANSACTIONS.md). The live INCOME terms differ from the demo's: the v4 deployer uses one stage (10 INCOME/USD, 2% cut per quarter indefinitely, 10% cash-out tax) and routes one Owner-chosen reserved percent to a single unlocked split held by the Owner; Sticky rewards are deferred. The demo drafts below keep their historical 75/15/10 and eight-cut assumptions.
...
For the Sticky portion, **hook = `JBTokenDistributor` and beneficiary = the FUND Sticky SHARE ERC20**, with `projectId = 0`. Neither the raw FUND token nor `StickyHook` is the beneficiary. Addresses and project IDs stay null until verified. Confirm distributor registration and actual recipient balances: a failed reserved-token hook can burn unconsumed tokens even when the parent payment/distribution succeeds.
...
Relevant local source: `JBController.queueRulesetsOf` (owner changes), `JBController.mintTokensOf` (mint permissions), `JBTerminalStore._tokenSurplusFrom` (payout reservations), `JBMultiTerminal.addToBalanceOf` (no new tokens), `JBMultiTerminal._cashOutTokensOf` (burn and fees), `StickyDeployer` (separate staking project), and `JBTokenDistributor` (reward routing and materialized vesting).
