...
Economic model specification, September 9, 2026. The hybrid combines an initial INCOME allocation with ongoing FUND-holder rewards. It supersedes the earlier premint-only, fixed-payoff, burn-to-convert and net-revenue designs. Founder Haus remains the concrete house example: its modeled revenue is rent and its customers are renters. The same framing can describe other income-producing assets, but asset ownership and enforceable rights require their own verified arrangements.

**Status (September 19, 2026): this is the illustrative demo model, not the live terms.** The contracts are documented in [TRANSACTIONS.md](TRANSACTIONS.md), [INCOME_INTEGRATION.md](INCOME_INTEGRATION.md) and [INCOME_RELEASE.md](INCOME_RELEASE.md). Live terms: every FUND launches through `HomerunDeployer` with its ERC-20 (no credits) and an owner-managed payment allowlist; the initial 500,000 INCOME goes to every FUND holder at the published snapshot with no staking or vesting; INCOME issues at 10 per USD, cuts 2% every quarter indefinitely, and taxes cash outs 10%; the Owner chooses one reserved percent (planning default 80%) and holds that whole split until redirecting it. Ongoing Sticky rewards are deferred ([STICKY_REWARDS.md](STICKY_REWARDS.md)). The comparisons below keep the demo's historical assumptions and numbers (75/15/10, eight 5% cuts, immediate unstaked rewards) and do not describe the deployed contracts.
