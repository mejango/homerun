...
The September 10 live integration and accepted reward policy are documented in [TRANSACTIONS.md](TRANSACTIONS.md), [INCOME_INTEGRATION.md](INCOME_INTEGRATION.md) and [STICKY_REWARDS.md](STICKY_REWARDS.md). Initial 500,000 INCOME includes every FUND holder at the published snapshot, including inactive wallet balances and unclaimed credits, with no staking or vesting requirement. Ongoing rewards use stock Sticky SHARE snapshots and four weekly vesting rounds; there is no minimum staking period or age multiplier. The comparisons below retain historical model assumptions and numbers, including immediate unstaked rewards, and do not override the accepted live policy. The current Create and demo integration briefs explicitly distinguish their estimates from that policy.
