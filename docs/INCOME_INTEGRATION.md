...
`HomerunDeployer` performs the following in one non-reentrant transaction:

1. Require a FUND launched by this deployer (`isFund`), the actual FUND NFT owner, correct creation fee, closed FUND with its verified empty canonical omnichain hook, finished owner minting, no pending reserved tokens or future rulesets, and the canonical FUND ERC-20.
2. Deploy INCOME through the six-argument canonical REVDeployer overload with the identical ordered global auto-issuance list on every chain. Counts sum to 500,000 and each beneficiary is the same deterministic helper address; stock REV records only the current chain’s count.
3. Resolve the first queued stage ID and execute `REVOwner.autoIssueFor`, verifying the complete local allocation exists as ERC-20 tokens and no local initial or reserved issuance remains pending. A zero-allocation chain skips minting. Late launches use the original stage ID even after the final stage has begun.
4. Deploy `HomerunInitialIncomeVault` with immutable actual INCOME identity, snapshot/domain/root, manifest commitment and factory; transfer the complete local allocation into it. Verify exact vault funding and zero helper residue before returning.
5. Verify canonical INCOME ownership/operator identity and record the permanent FUND→INCOME and FUND→vault mappings. A reserved mapping value prevents duplicate launch and reentry during external calls.
