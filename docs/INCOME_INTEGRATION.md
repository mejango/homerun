...
The helper exposes `LAUNCH_VERSION() = 2`. Its nine-argument `deployIncome` appends a nonzero `address operator` after `suckerConfiguration`. The caller must still own FUND, and `config.operator` remains that caller; the appended address sets the initial, unlocked INCOME incentive split beneficiary. The Owner can replace this beneficiary at any time through the stock controller’s split-group controls, and can edit any other split recipient or allocation. No split is locked. Each current or scheduled ruleset namespace is updated separately on each chain. The existing `IncomeDeployed.operator` event field continues to identify the control wallet. The client checks version 2 on every participating network before freezing a new plan and before preparing an unfinished deployment.

Recovery accepts the original eight-argument selector and the new selector, preserving saved pending/executed calldata byte for byte. A legacy frozen plan without an explicit incentive recipient retains each local FUND owner as its recipient. Restoring it never substitutes a newly entered Operator wallet. New plans freeze the initial recipient for each unfinished deployment. Authorized split changes on an already-launched sibling do not block later chains from completing the original launch plan.
...
The initial 500,000 INCOME is allocated proportionally to a fixed FUND ownership snapshot, including inactive ERC-20 balances, unclaimed credits, and Owner FUND. Claiming requires no FUND deposit, delegation, burn, vesting period, or continuing FUND balance. A later FUND transfer does not move this one-time entitlement.
...
Default new issuance is 70% Operator, 10% Sticky SHARE holders, and 20% payer beneficiary. The reserved bucket is 80%; its internal weights are 875,000,000 and 125,000,000 out of 1,000,000,000. The reward row uses `hook = JBTokenDistributor`, `beneficiary = verified SHARE ERC-20`, and project ID zero. Neither raw FUND nor the Sticky hook is that beneficiary. Every split row has `lockedUntil = 0`; the Owner may edit both the Operator and FUND-staker allocations and destinations. Operator changes affect subsequent reserved distributions, including tokens that have accrued but have not yet been distributed; already issued tokens remain with their holders. User-selected allocations remain supported, including an explicit zero-customer share that payment review discloses.

Both reserved split rows are locked through `type(uint48).max` in every stage. This preserves both recipients even if the Revnet control wallet later changes. A successful parent payment alone does not prove its reserved reward hook delivered tokens; actual balances/distributor funding must be checked.
...
The helper and vault are new, unaudited integration contracts, not verified production deployments. Installed SDK records do not yet contain verified Homerun helper, Sticky and distributor deployments for this flow. Separate Owner/Operator launches require a newly verified version-2 helper deployment and its SDK registration; an earlier helper does not accept the new selector. Simulation/test artifacts are not production deployment evidence. Source review, verification and registry publication remain prerequisites; no agent wallet has deployed these contracts or moved user funds.
