...
export function globalDraft(manifest = globalManifest()): IncomeGlobalLaunchDraft { return { version: 1, root: { chainId: 8453, projectId: '7' }, helper: HELPER, manifestUri: 'ipfs://global-snapshot', manifestHash: fundGlobalManifestHash(manifest), sourceSetHash: manifest.sourceSetHash, launchSalt: manifest.launchSalt, name: 'Homerun INCOME', ticker: 'RENT', metadataUri: 'ipfs://global-metadata', startsAtOrAfter: 1000, reservedBps: 8000, chains: manifest.allocations.map(local => ({ chainId: local.chainId, fundProjectId: local.fundProjectId, initialIncomeAmount: local.incomeAmount })) } }
...
export function launchPlan(input: Parameters<typeof prepareIncomeLaunch>[1]): PreparedIncomeLaunch {
  ...
  return { fund: fundState(input.chainId), manifest, snapshot, localAllocation, manifestHash: fundGlobalManifestHash(manifest), creationFee: 1n, startsAtOrAfter: input.startsAtOrAfter, configurationSalt: SALT, expectedConfigurationHash: SALT,
    request: { chainId: input.chainId, address: HELPER, abi: homerunDeployerAbi, functionName: 'deployIncome', args: [input.fundProjectId, snapshot, { name: input.name, ticker: input.ticker, uri: input.projectUri, salt: input.salt }, input.reservedBps, input.startsAtOrAfter, parseSuckerDeployerConfig(input.chainId, manifest.allocations.map(local => local.chainId), [MappableAsset.USDC], { version: 6, bridge: 'ccip', salt: input.salt })], value: 1n } }
}
