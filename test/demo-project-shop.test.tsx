...
describe('demo shop editor', () => {
  it('keeps legacy FUND items separate from INCOME inventory, currency, edits and clearing', async () => {
    const fund = JSON.stringify({ version: 1, currency: 'ETH', items: [{ ...newDemoShopItem(), name: 'Founding supporter', price: '0.025' }] })
    ...
    expect(host.querySelector('.ds-card-price')?.textContent).toBe('100 USD')
    await click('Clear demo shop')
    await click('Clear demo shop')
    expect(JSON.parse(localStorage.getItem(incomeKey)!).items).toEqual([])
    await mount()
    expect(host.querySelector('.ds-card-price')?.textContent).toBe('0.03 ETH')
  })
