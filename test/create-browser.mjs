...
try {
  ...
  await check('Owner profiles stay distinct through review, download, and reload', async () => {
    const introduction = 'We steward the workshop for our members.\nOur <trust> owns the building & funds repairs.';
    await input('ownerName').fill('Workshop trust');
    await input('ownerIntroduction').fill(introduction);
    await input('ownerPhoto').setInputFiles(new URL('../web/assets/founder-haus/exterior.jpg', import.meta.url).pathname);
    await page.locator('.create-operator-photo-preview').waitFor({ state: 'visible' });
    assert.equal(await page.locator('.create-operator-photo-preview').evaluate(img => img.complete && img.naturalWidth > 0 && img.naturalWidth <= 800 && img.naturalHeight <= 800), true);
    await page.reload({ waitUntil: 'domcontentloaded' });
    await currentStep(0);
    ...
    assert.equal(await input('operatorName').inputValue(), 'Workshop team');
    for (let index = 0; index < 3; index++) await next();
    const profile = page.getByRole('region', { name: 'Owner introduction', exact: true });
    assert.equal(await profile.locator('.operator-profile-name').textContent(), 'Workshop trust');
    assert.equal(await profile.locator('.operator-profile-introduction').textContent(), introduction);
    assert.equal(await profile.locator('trust').count(), 0, 'Ownership introductions remain plain text.');
    assert.equal(await page.getByRole('region', { name: 'Operator introduction', exact: true }).locator('.operator-profile-name').textContent(), 'Workshop team');
    const setup = await downloadSetup();
    assert.equal(setup.owner.name, 'Workshop trust');
    assert.equal(setup.owner.introduction, introduction);
    assert.match(setup.owner.photo, /^data:image\/jpeg;base64,/);
    assert.equal(setup.owner.address, ownerWallet);
    assert.equal(setup.operator.name, 'Workshop team');
    await page.setViewportSize({ width: 320, height: 844 });
    await noOverflow();
    await a11y('Owner and Operator profile review on mobile');
    await shot('create-owner-review-mobile.png');
    await page.setViewportSize({ width: 1440, height: 1000 });
  });
