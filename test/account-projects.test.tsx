...
describe('account project discovery', () => {
  ...
  it('scopes owned projects to the explicit account and includes credits plus ERC-20 holdings of both project types', async () => {
    ...
    expect(section('Your projects').textContent).not.toContain('Somebody else')
  ...
  it('retains the last successful owned and held data after background index failures', async () => {
    ...
    await settle()
    expect(section('Your projects').textContent).toContain('Showing the last indexed data')
    expect(section('Your projects').querySelector('a[href="/project/1/7"]')).not.toBeNull()
  ...
  it('lists a published project the account owns alongside its indexed projects', async () => {
    ...
    const owned = section('Your projects')
  ...
  it('keeps testnet published projects out of the mainnet list', async () => {
    ...
    await render(ACCOUNT_A)
    expect(section('Your projects').textContent).not.toContain('Published Workshop')
  })
