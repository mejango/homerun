...
try {
  for (const width of [1440, 390, 320]) {
    ...
    await expect(more).toHaveAttribute('aria-expanded', 'true')
    await expect(tabs.getByRole('tab', { name: 'Extras', exact: true })).toBeVisible()
    await expect(tabs.getByRole('tab', { name: 'Operators', exact: true })).toBeVisible()
    await expect(page.getByRole('menu', { name: /More project sections/ })).toHaveCount(0)
    await expect(page.getByRole('menuitemradio')).toHaveCount(0)
    await tabs.getByRole('tab', { name: 'Extras', exact: true }).focus()
    await page.keyboard.press('ArrowRight')
    await expect(tabs.getByRole('tab', { name: 'Operators', exact: true })).toBeFocused()
    await expect(page.getByRole('tabpanel', { name: 'Operators', exact: true })).toBeVisible()
    const expandedHash = new URL(page.url()).hash
    await accessible(`Expanded project tabs ${width}`)
    await more.click()
    await expect(more).toHaveAttribute('aria-expanded', 'false')
    await expect(more).toHaveAttribute('aria-label', 'More project sections, current: Operators')
    await expect(tabs.getByRole('tab', { name: 'Extras', exact: true })).toHaveCount(0)
    await expect(tabs.getByRole('tab', { name: 'Operators', exact: true })).toHaveCount(0)
    await expect(page.getByRole('tabpanel', { name: 'Operators', exact: true })).toBeVisible()
    assert.equal(new URL(page.url()).hash, expandedHash, 'Collapsing optional tabs preserves the selected section')
    await expect(more).toBeFocused()
    await page.keyboard.press('Enter')
    await expect(tabs.getByRole('tab', { name: 'Operators', exact: true })).toHaveAttribute('aria-selected', 'true')
    await more.click()
    await tab('Overview')
    ...
    console.log(`PASS ${width}px: six project sections, simultaneous accounts, inline tab expansion, funding graph, logo, retained payment, projections and accessibility`)
  }
